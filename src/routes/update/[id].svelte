...
<script>
    import { UpdateMangas, fetchUpdateManga } from "../../mangameestore"
    import MangaCard from "../../components/mangaCard.svelte"
    import { goto } from '$app/navigation';
    export let id

    let pageNumber = parseInt(id);
    $: fetchUpdateManga(pageNumber)
...
<div class="grid grid-cols-2 md:grid-cols-6 gap-5">
    {#each $UpdateMangas as UpdateManga}
        <a href={`/manga/${UpdateManga.MangaLink}`}>
            <MangaCard dataManga = {UpdateManga}/>
        </a>
    {/each}
</div>
